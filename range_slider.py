...
#       slide bounds
        ...
        #    -2*self.range_slider_token_halfwidth - \
        #    self.range_slider_token_LHS_offset\
        #    -self.range_slider_token_RHS_offset
        
        self.range_slider_maxRHS = self.range_slider_len - \
            self.range_slider_token_halfwidth - \
            self.range_slider_token_RHS_offset

        self.range_slider_minLHS = self.range_slider_token_halfwidth+\
             self.range_slider_token_LHS_offset
        
        self.range_slider_value_range = self.range_slider_maxRHS- \
            self.range_slider_minLHS

#       create a canvas for drawing the range-slider...
        self.canvas_frame = tk.Frame( self.master) 
        self.canvas = tk.Canvas(self.canvas_frame, 
            width=self.range_slider_len, 
            height=self.range_slider_height, 
            bg='white')
...
#       reate horizontal line sepcifying the slider token track
        self.canvas.create_line( ( \
            ...
            self.range_slider_token_vertical_offset+\
                self.range_slider_token_halfheight, 
            self.range_slider_len-self.range_slider_token_RHS_offset, 
            self.range_slider_token_vertical_offset+\
                self.range_slider_token_halfheight) )# fill='white')
...
#       SLIDER TOKENS 
        ...
        self.canvas.tag_bind("token", 
            "<B1-Motion>", 
            self.on_token_motion)
        
        self._pack_widgets()

    def _pack_widgets(self):
        self.entry_minval.pack(side=tk.LEFT)
        self.canvas.pack(expand=True, side=tk.LEFT, padx=0)
        self.canvas_frame.pack(side=tk.LEFT,)
        self.entry_maxval.pack(side=tk.LEFT)

    def _create_min_max_entry(self):
        self.entry_minval = tk.Entry(self.master, 
            bd=2, 
            relief=tk.GROOVE, 
            width=10)
        self.entry_maxval = tk.Entry(self.master, 
            bd=2, 
            relief=tk.GROOVE, 
            width=10)
    ...
    def _get_min_max_vals(self ,event):
        ...
        left_x = self.range_slider_value_range * \
            (self.minval-dmin)/(float(dmax)-dmin) \
                + self.range_slider_minLHS
       
        right_x = self.range_slider_value_range *\
            (self.maxval-dmin)/(float(dmax)-dmin) + \
            self.range_slider_minLHS  
        
        coors_right = self.canvas.coords(self.items[1])
        
        left_item = self.items[0]
        right_item = self.items[1]

            
        left_y = self.range_slider_token_vertical_offset+\
                self.range_slider_token_halfheight
        ...
        #   move the label
        self.canvas.itemconfig( self.minvaltext, 
            text="%.2f"%self.minval)
        
        self.canvas.coords( self.minvaltext, 
                    (left_x-self.text_offset_LHS, 
                    self.token_text_vertical_position))
        
        
        right_y = self.range_slider_token_vertical_offset+\
                self.range_slider_token_halfheight
        ...
        # move the label
        self.canvas.itemconfig( self.maxvaltext, 
            text="%.2f"%self.maxval)
        
        self.canvas.coords( self.maxvaltext, 
            (right_x+self.text_offset_RHS, 
                self.token_text_vertical_position))
...
#       TEXT LABEL
    ...
    def _create_token(self, coord, color):
        ...
        (x,y) = coord
        return self.canvas.create_rectangle(x-\
                self.range_slider_token_halfwidth, 
    ...
    def on_token_motion(self, event):
        ...
        if clicked_x > other_x:
            ...
            self.canvas.coords( self.maxvaltext, 
                (x_new+self.text_offset_LHS,
                self.token_text_vertical_position))
            new_drag = min( self.range_slider_maxRHS,  event.x)
            new_drag = max(  other_x , new_drag)
            self._drag_data["x"] = new_drag 
            
            val_range = float(self.dmax-self.dmin)
            pix_range = float(self.range_slider_value_range)
            
            self.maxval = (x_new / pix_range )*val_range + self.dmin
            ...
            self.minval = (x_new / pix_range )*val_range + self.dmin
